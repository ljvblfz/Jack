...
public class ReflectFactory<T> {
  ...
  public ReflectFactory(@Nonnull Class<? extends T> cls, boolean respectAccessibility,
      @Nonnull Class<?>... argsType) {
    assert checkAssertion(cls);

    this.cls = cls;
    try {
      ctor = cls.getDeclaredConstructor(argsType);
      if (!respectAccessibility) {
        ctor.setAccessible(true);
      }
    } catch (NoSuchMethodException e) {
      throw new AssertionError("Constructor '" + toString(cls, argsType) + "' is not found");
    }
  }

  @Nonnull
  private StringBuilder toString(@Nonnull Class<? extends T> cls, @Nonnull Class<?>... argsType) {
  ...
  @Nonnull
  public T create(@Nonnull Object... args) {
    try {
