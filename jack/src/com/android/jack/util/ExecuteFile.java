...
public class ExecuteFile {
  ...
  public int run() throws Exception {
    ...
    if (localInStream != null) {
      suckIn = new Thread(
          new ThreadByteStreamSucker(localInStream, proc.getOutputStream()));
    } else {
    ...
    if (localOutStream != null) {
      if (localOutStream instanceof PrintStream) {
        ...
        suckOut = new Thread(
            new ThreadByteStreamSucker(proc.getInputStream(), localOutStream));
      }
    }
    ...
    if (localErrStream != null) {
      if (localErrStream instanceof PrintStream) {
        ...
        suckErr = new Thread(
            new ThreadByteStreamSucker(proc.getErrorStream(), localErrStream));
      }
    }
