  ...
  private static class Visitor extends JVisitor {
    ...
    public boolean visit(@Nonnull JLambda lambdaExpr) {
      ...
      for (JVariableRef capturedVarRef : lambdaExpr.getCapturedVariables()) {
        ...
        if (alreadyCapturedVar != null) {
          tr.append(
              new Replace(capturedVarRef, alreadyCapturedVar.makeRef(capturedVar.getSourceInfo())));
        }
      ...
      for (CapturedVarToLiberatedVar varToLiberate : Lists.reverse(varToLiberates)) {
        tr.append(
            new PrependStatement(lambdaMethodBody.getBlock(),
                new JAsgOperation(SourceInfo.UNKNOWN,
                    varToLiberate.liberatedVariable.makeRef(SourceInfo.UNKNOWN),
                    new JLiberateVariable(SourceInfo.UNKNOWN, closure.makeRef(SourceInfo.UNKNOWN),
                        varToLiberate.paramCapturingVar.makeRef(SourceInfo.UNKNOWN)))
                            .makeStatement()));
      }
    ...
    public void endVisit(@Nonnull JVariableRef varRef) {
      ...
      if (liberatedVariable != null) {
        tr.append(new Replace(varRef, liberatedVariable.makeRef(varRef.getSourceInfo())));
      }
    }
  }
